...
/**
 * Lab3 aim to find the maximum and minimum values of a large matrix by splitting it into smaller 
...
//		Double globalMax = finderArray[1].getMax();
		sharedObject=new SharedObject(finderArray[0].getMin(),finderArray[1].getMax());
		for(int i=0;i<nRows;i++) {
			if(finderArray[i].getMin() < sharedObject.getGlobalMin()) {
				sharedObject.setGlobalMin(finderArray[i].getMin());
			}
			if(finderArray[i].getMax() > sharedObject.getGlobalMax()) {
				sharedObject.setGlobalMax(finderArray[i].getMax());
			}
		}

		System.out.println("Global min: " + sharedObject.getGlobalMin());
		System.out.println("Global max: " + sharedObject.getGlobalMax());
